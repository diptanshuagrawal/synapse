#!/usr/bin/env python3
"""
confluence_validate.py — sanity-check Confluence-ingested data in events.db.

v1 checks (will grow iteratively):
  1. attribution — actors not in any people.yaml entry (scope team/slice/external)

Note on confluence actor shape
------------------------------
Confluence ingest writes `actor` as the Atlassian account-id (24-hex old form,
or "712020:uuid" new form). people.yaml has `jira_id:` set to that value, so
canonicalisation works for team members already in yaml.
...
Severity policy
---------------
- FAIL : raw_unknown actor with > 20 page events (confluence is lower-volume)
- WARN : any raw_unknown
- PASS : zero raw_unknown

CLI
---
    .venv/bin/python derive/confluence_validate.py            # human-readable
    .venv/bin/python derive/confluence_validate.py --json     # for cron-status
"""
...
UNKNOWN_FAIL_COUNT = 20  # confluence is lower-volume than github/jira
...
def compute(conn: sqlite3.Connection) -> dict:
    ...
    n_total = conn.execute(
        "SELECT COUNT(*) FROM events WHERE source='confluence'"
    ).fetchone()[0]
    ...
    if not raw_unknown:
        findings.append(["PASS", "attribution", "all actors scoped (team/slice/external)"])
    else:
        biggest = raw_unknown[0]
        sev = "FAIL" if biggest[1] > UNKNOWN_FAIL_COUNT else "WARN"
        sample = ", ".join(f"{a[:30]}({n})" for a, n in raw_unknown[:3])
        findings.append([
            sev, "attribution",
...
def _render_human(report: dict) -> None:
    print(f"\n=== confluence_validate · {report['computed_at']} ===")
