...
4. **Daily routine** — built + live: `derive/service_derive/refresh-skeletons.sh` (deterministic skeleton refresh + content-diff gate over `build_skeletons.py`/`go_extractor.py`) feeds the `service-brief-daily` routine in `scheduled-tasks/routines.yaml`, re-briefing only structurally-changed services. Resolved 2026-07-08: the script now probes `python3 -c "import yaml"` at startup, falls back to `work-context/.venv/bin/python3`, and routes every python invocation through the resolved interpreter (fails loud if neither has yaml) — unattended fires no longer die on `import yaml`. refresh-embeddings stays owner-invoked.
