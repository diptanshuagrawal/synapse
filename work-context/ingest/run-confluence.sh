...
# makes Jira/Confluence return HTTP 200 with an EMPTY result (not a 401), so a
# placeholder fallback silently ingests nothing while the run still "succeeds" —
# data freezes with no error. Refuse to auth as a placeholder.
