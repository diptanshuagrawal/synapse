...
#
# Runs slack_discover_channels.py in --auto-mode + --apply: appends
# `auto_full` and `auto_team_involved` rows to config/slack_channels.yaml,
# writes proposals JSON for cron-status DISCOVERY block, and validates
# the resulting yaml. Next slack-ingest fire (every :00/:30 IST 12-22h)
# auto-bootstraps newly-added channels from now-365d via PAGE_CAP catch-up.
#
# Safety rails kept:
