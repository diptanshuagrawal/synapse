...
// Slack mrkdwn → HTML, so the MoM preview looks exactly like the pasted Slack
// message: *bold*, _italic_, `code`, <url|label> links, • / - bullets. Escapes
// HTML first, then restores Slack <…> links (kept separate from md() so notes,
// which use **bold**/##/[](), render unchanged).
function smd(t){
 ...
 return (t||'').split('\n').map(line=>{
   ...
   if(/^\s*[•\-]\s+/.test(line)) return '<div style="margin:1px 0 1px 4px">• '+h.replace(/^\s*[•\-]\s+/,'')+'</div>';
   if(!line.trim()) return '<div style="height:8px"></div>';
   return '<div style="margin:2px 0">'+h+'</div>';
 }).join('');
}
async function poll(){
...
function render(){
 ...
 <div id="content">${
   ...
   : tab==='MoM' ? (detail.mom?`<div class="md">${smd(detail.mom)}</div>`
   ...
   : tab==='Share' ? `<div style="background:var(--recbg);border:1px solid var(--recline);border-radius:12px;padding:10px 16px;margin-bottom:14px;font-size:13px;display:flex;flex-wrap:wrap;gap:10px;align-items:center">
     ...
     <div class="md">${(detail.share_src==='MoM'?smd:md)(detail.share)}</div>`
   : tab==='Speakers' ? (function(){
       const sp=detail.speakers;
       // Dropdown = roster + a "custom name" escape hatch for anyone not in
       // people.yaml (external attendees etc). A stored custom pick has a slug
       // handle that isn't in the roster, so surface it as its own selected
       // option (labelled with the typed name) — otherwise it'd read blank.
       const opts=(selh,selname)=>{
         const inRoster=sp.roster.some(r=>r.handle===selh);
         return ['<option value="">— unassigned —</option>']
           .concat((selh&&!inRoster)?['<option value="'+esc(selh)+'" selected>'+esc(selname||selh)+'</option>']:[])
           .concat(sp.roster.map(r=>`<option value="${esc(r.handle)}" ${r.handle===selh?'selected':''}>${esc(r.name)}</option>`))
           .concat('<option value="__custom__">+ add a name…</option>').join('');
       };
       return `<div style="font-size:12.5px;color:var(--muted);margin-bottom:12px">Assign each detected voice to a person. Not in the list? Pick <b>+ add a name…</b>. Your choice is saved as ground truth for the note AND teaches Steno to recognise that voice in future meetings. 🔊 = auto-matched by voice.</div>`+
         sp.list.map(s=>`<div style="display:flex;align-items:center;gap:12px;padding:9px 12px;border:1px solid var(--line);border-radius:10px;margin-bottom:8px">
           <b style="min-width:76px">${esc(s.display)}</b>
           <span style="flex:1;color:var(--muted);font-size:12.5px">${s.name?('→ <b style="color:var(--accent)">'+esc(s.name)+'</b>'):(s.suggestion?('🔊 sounds like <b>'+esc(s.suggestion)+'</b> ('+s.score+') <button onclick="setSpeaker(\''+s.cluster+'\',\''+esc(s.suggestion_handle)+'\')" style="border:1px solid var(--recline);background:var(--card);color:var(--accent);border-radius:7px;padding:2px 9px;font-size:11.5px;cursor:pointer;margin-left:4px">confirm</button>'):'unassigned')}</span>
           <select onchange="setSpeaker('${s.cluster}',this.value)" style="border:1px solid var(--line);background:var(--card);color:var(--ink);border-radius:8px;padding:5px 10px;font-size:12.5px">${opts(s.handle||'', s.name||'')}</select>
         </div>`).join('');
     })()
...
async function setSpeaker(cluster,handle){
 // "+ add a name…" → free-text tag for someone not in the roster. Sends {name};
 // the server slugifies it into a stable handle and enrolls the voiceprint.
 if(handle==='__custom__'){
   const name=await promptModal('Tag this speaker',
     'Name someone not in your roster (e.g. an external attendee). Steno saves it for this note and learns the voice for future meetings.','','Save');
   if(name==null||!name.trim()){seg_(sel);return;}  // cancelled → re-render to reset the <select>
   await fetch('/api/speakers/'+sel,{method:'POST',body:JSON.stringify({cluster:cluster,name:name.trim()})});
   seg_(sel);return;
 }
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if p == "/api/scratchpad":
            ...
            # Regenerate = archive the current note + drop a .regen.request
            # marker. The meeting-notes-auto routine picks up the marker on its
            # next fire REGARDLESS of the meeting's age — unlike the "10 most
            # recent recordings" scan, which never sees an old meeting, so a
            # regen of anything older than the last ~10 recordings used to be
            # dropped silently. Old version kept as .md.prev. (Instant path:
            # ask Claude to run /meeting-notes.)
            mid = re.sub(r"[^a-zA-Z0-9_-]", "", p.rsplit("/", 1)[1])
            note = NOTES_DIR / f"{mid}.md"
            if mid:
                if note.exists():
                ...
                (NOTES_DIR / f"{mid}.regen.request").write_text("", encoding="utf-8")
            self._json({"ok": True})
            ...
            # body = {"cluster":"SPEAKER_00","handle":"alex"}  (roster pick),
            #   or {"cluster":...,"name":"Sam Rivera"} (custom, non-roster),
            #   or neither (clears the assignment).
            mid = re.sub(r"[^a-zA-Z0-9_-]", "", p.rsplit("/", 1)[1])
            month = mid[:7]
            try:
            ...
            custom = str(req.get("name", "")).strip()  # free-text tag for a non-roster person
            ...
            if cluster not in data:
                ...
                return
            if handle:  # roster pick (or confirming a voice suggestion)
                data[cluster]["handle"] = handle
                data[cluster]["name"] = _name_for(handle, _roster())
            elif custom:  # "+ add a name…" — slugify into a stable voiceprint
                # handle so the voice re-matches next time; keep the typed name verbatim.
                handle = re.sub(r"[^a-z0-9]+", "-", custom.lower()).strip("-") or "guest"
                data[cluster]["handle"] = handle
                data[cluster]["name"] = custom
